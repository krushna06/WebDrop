...
import WebDropServer from "./server.js";
import WebDropWsServer from "./ws-server.js";
...
// Start a new webdrop instance when an uncaught exception occurs
if (conf.autoStart) {
    process.on(
        'uncaughtException',
        () => {
...
const webDropServer = new WebDropServer(conf);

if (!conf.signalingServer) {
    // Start websocket server if SIGNALING_SERVER is not set
    new WebDropWsServer(webDropServer.server, conf);
} else {
...
console.log('\nWebDrop is running on port', conf.port);
